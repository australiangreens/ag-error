 ...
 */
export function getPrototypeChainOf(obj: unknown, options = {}) {
  const opts = { ...DEFAULT_GET_PROTOTYPE_CHAIN_OF_OPTIONS, ...options };

  if (!obj) {
    if (obj === null && opts.includeNull) return [null];
    return [];
  }
  ...
  do {
    if (opts.stopBefore && opts.stopBefore(nextPrototype)) break;
    prototypeChain.push(nextPrototype);
    if (opts.stopAfter && opts.stopAfter(nextPrototype)) break;
  ...
  if (opts.includeNull) prototypeChain.push(null);
 ...
 */
export function getClassChainOf(obj: unknown, options = {}) {
